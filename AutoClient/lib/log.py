...
class Logger(object):
    ...
    @staticmethod
    def check_method_exist(log_abs_path):
        log_path = os.path.split(log_abs_path)[0]
        if not os.path.exists(log_path):
            os.mkdir(log_path)

    def initialize_run_log(self):
        self.check_method_exist(self.run_log_file)
        file_1_1 = logging.FileHandler(self.run_log_file,'a',encoding='utf8')
    ...
    def initialize_error_log(self):
        self.check_method_exist(self.error_log_file)
        file_1_1 = logging.FileHandler(self.error_log_file, 'a', encoding='utf8')
        fmt = logging.Formatter(fmt='%(asctime)s - %(levelname)s: %(message)s')
        file_1_1.setFormatter(fmt)
